...
class Konfirmasi_model extends CI_Model
{
	...
	function ambil_data()
	{
		$this->db->select('konfirmasi.id_konfirmasi, customer.nama, memesan.id_memesan, memesan.jumlah, konfirmasi.gambar');
        $this->db->from('konfirmasi');
        $this->db->join('customer', 'konfirmasi.id_customer = customer.id_customer');
        $this->db->join('memesan', 'konfirmasi.id_memesan = memesan.id_memesan');
		$query = $this->db->get();
		return $query->result();
	}
